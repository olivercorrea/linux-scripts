...
remove_application() {
    # Obtener la lista de aplicaciones instaladas con cargo
    installed_apps=$(cargo install --list)  # Comando que lista todas las aplicaciones instaladas con cargo

    # Si no hay aplicaciones instaladas con cargo
    if [[ -z "$installed_apps" ]]; then
        echo -e "${RED}❌ No hay aplicaciones instaladas con cargo.${NC}"
        return
    fi

    # Mostrar las aplicaciones instaladas con cargo
    echo -e "${GREEN}✔️ Aplicaciones instaladas con cargo:${NC}"
    ...
    app_name=$(echo "$installed_apps" | sed -n "${app_number}p" | awk '{print $1}')  # Extraer el nombre de la app
