...
remove_application() {
    # Obtener la lista de paquetes instalados con apt
    installed_apps=$(dpkg --get-selections | grep -v deinstall | awk '{print $1}')  # Filtramos solo los paquetes instalados

    # Si no hay aplicaciones instaladas con apt
    if [[ -z "$installed_apps" ]]; then
        echo -e "${RED}❌ No hay aplicaciones instaladas con apt.${NC}"
        return
    fi

    # Mostrar las aplicaciones instaladas con apt
    echo -e "${GREEN}✔️ Aplicaciones instaladas con apt:${NC}"
