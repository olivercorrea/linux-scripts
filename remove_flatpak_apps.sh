...
remove_application() {
    # Obtener la lista de aplicaciones instaladas con Flatpak
    installed_apps=$(flatpak list --app --columns=application)  # Listamos solo las aplicaciones

    # Si no hay aplicaciones instaladas con Flatpak
    if [[ -z "$installed_apps" ]]; then
        echo -e "${RED}❌ No hay aplicaciones instaladas con Flatpak.${NC}"
        return
    fi

    # Mostrar las aplicaciones instaladas con Flatpak
    echo -e "${GREEN}✔️ Aplicaciones instaladas con Flatpak:${NC}"
    ...
    app_name=$(echo "$installed_apps" | sed -n "${app_number}p")  # Extraer la aplicación seleccionada por su índice
